...
describe("MintForestV1", function () {
  ...
  describe("Save Message", function () {
    it("Should't date verified", async () => {
      ...
      await expect(
        contractCaller.signin(
          signParams,
          publicUser.signTypedData(
            domain,
            {
              SigninParams: [
      ...
      await expect(
        contractCaller.signin(
          ...
          signer.signTypedData(
            ...
            { user: publicUser.address, ...signAgainParams }
          )
        )
      ).to.be.revertedWithCustomError(contract, "DuplicateData");
    });

    it("ME not be zero", async () => {
      const { contract, publicUser, domain } = await loadFixture(deployFixture);
      const contractCaller: any = contract.connect(publicUser);
      const { signer } = await setRightSigner(contract);
      const signParams = {
      ...
      ).to.be.revertedWithCustomError(contract, "InvalidPoint");
    });

    describe("Simulate real user behavior", () => {
      it("User daily signin", async () => {
        const { contract, publicUser, domain } = await loadFixture(
          deployFixture
        );
        const contractCaller: any = contract.connect(publicUser);
        const { signer } = await setRightSigner(contract);
        ...
        await expect(
          await contract.signinRecord(publicUser.address, signParams.time)
        ).to.be.equal(signParams.point);
      });
      ...
      it("User steal other unclaimed ME", async () => {
        ...
        await expect(
          await contract.stealRecord(stealParams.target, stealParams.time)
        ).to.be.equal(stealParams.point);
      });
      ...
      it("User open reward", async () => {
        ...
        await contractCaller.openReward(
          rewardParams,
          signer.signTypedData(
            domain,
            {
        ...
        await expect(
          await contract.rewardRecord(publicUser.address, rewardParams.rewardId)
        ).to.be.equal(rewardParams.point);
      });
      ...
      it("User turntable", async () => {
        ...
        await contractCaller.turntable(
          turntableParams,
          signer.signTypedData(
            domain,
            {
        ...
        await expect(
          await contract.turntableRecord(
            publicUser.address,
            turntableParams.time,
            turntableParams.count
          )
        ).to.be.equal(turntableParams.point);
      });
    });
  });
});
